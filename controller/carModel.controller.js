...
const userService = require('../service/carModel.service');
const db = require('_helpers/db');
const User = db.carModel;
...
router.get('/select-category/:id', selectCarCategory);
...
async function selectCarCategory(req, res){
    try {
        const cond={
            carCategory:req.params.id
        }
        const userCars = await User.find(cond);     
        // if(userCars.carCategory.image) userCars.carCategory.image = await getSignedUrl(userCars.carCategory.image);
        return res.status(200).json(userCars);
        // return res.status(200).json({ status:'success', data: userCars });
    } catch (error) {
