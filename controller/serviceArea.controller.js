...
const userService = require('../service/serviceArea.service');
const db = require('_helpers/db');
const User = db.ServiceArea;
...
//     try {
//         const serviceArea = await User.findById(req.params.id);        
 
//         return res.status(200).json(serviceArea);
//         // return res.status(200).json({ status:'success', data: userCars });
//     } catch (error) {
//         return res.status(400).json({ status:'error', error: error.message });
//     }
...
async function getAll(req, res){
    try {
        const serviceArea = await User.find()
